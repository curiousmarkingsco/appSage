            ...
            <div class="container mx-auto p-4">
                ...
                <header class="flex flex-col md:flex-row items-center justify-between rounded-lg p-4 bg-white dark:bg-slate-900 dark:text-white shadow">
                    ...
                    <div class="flex items-center space-x-2 mt-4 md:mt-0">
                        <input id="search-input" type="text" placeholder="Search notes..." class="px-3 py-1 border rounded-lg" />
                        <div class="relative">
                            <button id="search-criteria-btn" class="px-3 py-1 border rounded-lg">Search by</button>
                            <div id="search-criteria-dropdown"
                ...
                <div id="modal-labels" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 dark:bg-opacity-30 hidden">
                    <div class="bg-white dark:bg-black dark:text-white rounded-lg p-4 w-80 dark:border dark:border-slate-500">
                        ...
                        <!-- List of labels -->
                        <div id="label-list" class="max-h-40 overflow-y-auto mb-2">
                ...
                <div id="modal-color" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 dark:bg-opacity-30 hidden">
                    <div class="bg-white dark:bg-black dark:text-white rounded-lg p-4 w-80 dark:border dark:border-slate-500">
                        ...
                        <div class="mb-2">
                            ...
                            <input id="bg-color" type="color" class="w-full" />
                        </div>
                        <div class="mb-2">
...
function initializeQuickNotes(container) {
    ...
    const noteEditorModal = document.getElementById('note-editor-modal'); // The modal wrapper for the editor
    const noteEditor = document.getElementById('note-editor');            // The actual note editor snippet within the modal
    ...
    const storedNotes = JSON.parse(localStorage.getItem('quickNote.notes')) || [];
    ...
    const pinnedContainer = document.querySelector('#pinned-notes .notes-grid');
    const activeContainer = document.querySelector('#active-notes .notes-grid');
    const archivedContainer = document.querySelector('#archived-notes .notes-grid');
    ...
    if (noteId) {
        ...
        // (Optionally, re-render media gallery and label pills)
        // renderMediaGallery();
        updateLabelPills();
        }
    } else {
    ...
    noteEditorModal.querySelector('#bg-close-editor').addEventListener('click', () => {
        closeNoteEditor();
    });
    if (mode === 'text') {
        document.getElementById('note-content').focus();
    } else if (mode === 'checkbox') {
        const firstInput = document.querySelector('#checkbox-list input[type="text"]');
    ...
    const note = {
        ...
        pinned: isPinned,
        archived: document.getElementById('toggle-archive').classList.contains('archived') || false
    };

    let storedNotes = JSON.parse(localStorage.getItem('quickNote.notes')) || [];
    if (!currentNoteId) {
    ...
    document.getElementById('close-modal-image').addEventListener('click', () => { toggleModal('modal-image', false); });
    ...
    const addAud = document.getElementById('add-audio');
    if (addAud) addAud.addEventListener('click', () => { toggleModal('modal-audio', true); });
    document.getElementById('close-modal-audio').addEventListener('click', () => { toggleModal('modal-audio', false); });
    ...
    const pillsContainer = document.getElementById('label-pills');
    ...
    const mngLbl = document.getElementById('manage-labels');
    if (mngLbl) mngLbl.addEventListener('click', () => {
    renderLabelList();
    toggleModal('modal-labels', true);
    });
    document.getElementById('close-modal-labels').addEventListener('click', () => {
    toggleModal('modal-labels', false);
    });
    document.getElementById('save-labels').addEventListener('click', () => {
    ...
    // -------------------------
    const chsClr = document.getElementById('choose-color');
    if (chsClr) chsClr.addEventListener('click', () => { toggleModal('modal-color', true); });
    document.getElementById('close-modal-color').addEventListener('click', () => { toggleModal('modal-color', false); });
    ...
    const pnNte = document.getElementById('pin-note');
    ...
    const tglArc = document.getElementById('toggle-archive');
    if (tglArc) tglArc.addEventListener('click', () => {
    document.getElementById('toggle-archive').classList.toggle('archived');
    saveNote();
    });
    const elpMnu = document.getElementById('ellipsis-menu');
    if (elpMnu) elpMnu.addEventListener('click', () => {
    const dropdown = document.getElementById('dropdown-menu');
    dropdown.classList.toggle('hidden');
    });

    const dltNte = document.getElementById('delete-note');
    ...
    const dropdown = document.getElementById('dropdown-menu');
    const ellipsis = document.getElementById('ellipsis-menu');
    ...
    document.getElementById('create-text-note').addEventListener('click', () => {
    const modal = document.getElementById('note-editor-modal');
    modal.classList.remove('hidden');
    modal.dataset.mode = 'text';
    const textarea = modal.querySelector('#note-content');
    textarea.focus();
    });

    document.getElementById('create-checkbox-note').addEventListener('click', () => {
    const modal = document.getElementById('note-editor-modal');
    modal.classList.remove('hidden');
    modal.dataset.mode = 'checkbox';
    setTimeout(() => {
        const firstCheckbox = modal.querySelector('#checkbox-list input[type="text"]');
    ...
    document.getElementById('search-criteria-btn').addEventListener('click', () => {
    document.getElementById('search-criteria-dropdown').classList.toggle('hidden');
    });

    // ----- Label Filtering Dropdown -----
    document.getElementById('label-filter-btn').addEventListener('click', () => {
    ...
    document.getElementById('label-filter-search').addEventListener('input', function () {
    const query = this.value.toLowerCase();
    ...
    document.getElementById('label-filter-list').addEventListener('click', (e) => {
